...
export default function StatsSection() {
  ...
  const respondContactRatio = useMemo(() => {
    ...
    return (
      (venuesContactedThisMonth.length / venuesRespondedThisMonth.length) * 100
    );
    ...
    <>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {venuesRespondedThisMonth.length} /{" "}
              {venuesContactedThisMonth.length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {topVenue?.artistName ?? topVenue?.username}
            </div>
            <p className="text-muted-foreground text-xs">
              {bookings.filter((b) => b.requesterId === topVenueId).length}{" "}
              shows booked
            </p>
          </CardContent>
        </Card>
