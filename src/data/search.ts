...
export const searchClient = algoliasearch("GCNFAI2WB6", "c89ebf37b46a3683405be3ed0901f217");
...
export async function queryVenuesInBoundedBox(
  ...
  const formattedVenueGenreFilter =
    venueGenres != null ? `(${venueGenres.map((e) => `venueInfo.genres:'${e}'`).join(" OR ")})` : null;
  const formattedUnclaimedFilter = unclaimed != null ? `unclaimed:${unclaimed}` : null;
  ...
  try {
    ...
    const response = await usersIndex.search<UserModel>("", {
      filters: filtersStr,
      insideBoundingBox: bounds === null ? undefined : [[bounds.sw.lat, bounds.sw.lng, bounds.ne.lat, bounds.ne.lng]],
...
export async function queryUsers(
  ...
  const formattedLabelFilter =
    labels != null ? `(${labels.map((e) => `performerInfo.label:'${e}'`).join(" OR ")})` : null;
  const formattedGenreFilter =
    genres != null ? `(${genres.map((e) => `performerInfo.genres:'${e}'`).join(" OR ")})` : null;
  const formattedOccupationFilter =
    occupations != null ? `(${occupations.map((e) => `occupations:'${e}'`).join(" OR ")})` : null;
  const formattedOccupationsBlacklistFilter =
  ...
  const formattedLocationFilter = lat != null && lng != null ? `${lat}, ${lng}` : undefined;
